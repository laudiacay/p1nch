...
template Swap() {
    ...
    signal p2skh_hash <== ItemHasherSK()(1, p2skh_timestamp, sk, inp_tok_addr, p2skh_amount, P2SKH_INSTR, [0, 0], p2skh_randomness);
    signal _p2skh_comm <== Poseidon(2)([p2skh_hash, p2skh_comm_randomness]);
    _p2skh_comm === p2skh_comm;

    signal _new_swap_hash <== ItemHasherPK()(1, new_hash_timestamp, received_pk, inp_tok_addr, p2skh_amount, SWAP_INSTR, [out_tok_addr, 0], swap_randomness);
...
template SwapResolveToP2SKH() {
  ...
  // Check the swap event commitment
	signal _swap_utxo_hash <== ItemHasherSK()(1, swap_event_timestamp, sk, inp_tok, inp_tok_amount, SWAP_INSTR, [out_tok, 0], swap_randomness);
  ...
  // Check the output hash
	signal _out_p2skh_hash <== ItemHasherPK()(1, p2skh_timestamp, pk_out, out_tok, amount_out, P2SKH_INSTR, [0, 0], p2skh_randomness);
