...
app.post('/sequencer', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!data.ticketKey || !data.erc20ID || !data.amount || !data.wellFormedProof) {
      res.status(500).json({ message: 'missing required fields... check the api spec' });
      return;
    }
    ...
    res.json({ message: 'Data received and added to Redis. it will be on chain soon :)', receivedData: data });
  } catch (error) {
...
cron.schedule('*/5 * * * *', async () => {
  ...
  for (const itemString of items) {
    ...
    const aliceAuthorized = await erc20Contract.allowance(item.alice, process.env.CONTRACT_ADDRESS);
    if (aliceAuthorized < item.amount) {
      console.log("dropping item: alice didn't authorize sufficient funds", item);
      continue;
    }
